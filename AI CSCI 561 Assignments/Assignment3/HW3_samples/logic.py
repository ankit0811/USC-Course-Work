...
def fol_bc_or(KB, goal, theta):
    ...
    for rule in rules:
        print "OR: Rule is ", rule
        lhs, rhs = parse_definite_clause(standardize_variables(rule))
        # pdb.set_trace()
        thetaBeforeAnd = unify(rhs, goal, theta)
        print "OR: thetaBeforeAnd", thetaBeforeAnd
        thetasBeforeAND = fol_bc_and(KB, lhs, thetaBeforeAnd)
        for theta1 in thetasBeforeAND:
            print "OR: theta1", theta1
            yield theta1

def fol_bc_and(KB, goals, theta):
    if theta is None:
        # print "AND: pass as theta is None : ", theta
        pass
    elif not goals:
        # print "AND: yield theta as no goals : ", goals
        yield theta
    else:
        first, rest = goals[0], goals[1:]
        print "AND: before Subst ", first
        sen = subst(theta, first)
        print "AND: after subst ", sen
        thetas = fol_bc_or(KB, sen, theta)
        for theta1 in thetas:
            print "AND: theta1", theta1
            thetasForAND = fol_bc_and(KB, rest, theta1)
            for theta2 in thetasForAND:
                print "AND: theta2", theta2
                yield theta2
...
# print dpll_satisfiable((A|B)&(~C|~D|E)&(~A|D)& (~A|~E)&(~B|D)&(~B|~E))
print test_ask('Aunt(Jane, Shelly)', sample_KB)
